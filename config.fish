...
set -l PATHS $HOME/.gem/ruby/2.2.0/bin $HOME/scripts/bin \
             ...
             /usr/bin/core_prel $HOME/.local/bin
for i in $PATHS
...
ialias docker-pid "sudo docker inspect --format '{{.State.Pid}}'"
ialias docker-ip "sudo docker inspect --format '{{ .NetworkSettings.IPAddress }}'"
ialias dim "sudo docker images"
ialias dci "sudo docker commit"
ialias dps "sudo docker ps -a"
ialias drm "sudo docker rm"
ialias drmi "sudo docker rmi"
ialias drun "sudo docker run"
ialias dexe "sudo docker exec"
ialias dcom "sudo docker-compose"
ialias dpu "sudo docker push"
ialias dpua "for i in littleqz/{nginx,redis,php,mariadb}; sudo docker push \$i; end"
ialias dpla "for i in littleqz/{nginx,redis,php,mariadb}; sudo docker pull \$i; end"

# git
ialias gin "git init"
ialias gdf "git diff"
ialias gs "git status"
ialias gl "git log --color --graph --pretty=format:'%Cred%h%Creset -%C(yellow)%d%Creset %s %Cgreen(%cr) %C(bold blue)<%an>%Creset' --abbrev-commit"
ialias grl "git reflog"
ialias gc "git checkout"
ialias gm "git merge --no-ff"
